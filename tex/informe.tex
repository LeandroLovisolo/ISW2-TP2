...
\begin{tabular}{lp{13cm}l}
  ...
  \hline
  T01 & Diseño de la interfaz del usuario para autenticación & 8h \\
  T02 & Diseño de los requisitos mínimos de las contraseñas & 8h \\
  T02 & Implementación de CU4 & 9h \\
  T03 & Testing de CU4 & 9h \\
  \hline
\end{tabular}
...
\newpage
\section{Diagrama de componentes y conectores}
...
\begin{figure}[h!]
\centering
  \includegraphics[width=7cm]{./diagramas/referencias.png}
  \caption{Referencia de los conectores creados.}
  \label{fig:gantt}
\end{figure}

El \textit{Conector encriptado} le agrega la capacidad de cifrado al conector original.
Por otro lado, el \textit{Conector selector} funciona como un proxy y el mismo se encuentra modelado posteriormente.
Por último, el \textit{Conector asincrónico} es el visto en clase, a diferencia de que no conseguimos la flecha correspondiente para representarlo.

\subsection{Nodo regional}
\diagrama{./diagramas/nodoregional.pdf}

\subsection{Actuador web}
\diagrama{./diagramas/actuadorweb.pdf}

\subsection{Emisor de mensajes}
\diagrama{./diagramas/emisordemensajes.pdf}

\subsection{Receptor de mensajes}
\diagrama{./diagramas/receptordemensajes.pdf}

\subsection{Conector selector (proxy)}
